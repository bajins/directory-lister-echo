...
//新版markdown查看器
function newmd(url){
	var url = "./admin/viewmd.php?file=" + url;
	layer.open({
		title:'MarkDown查看器',
	  	type: 2, 
	  	area: ['80%', '80%'],
...
function video(url){
	...
	var videourl = "./admin/video.php?url=" + url;
	//layer.msg(videourl);
	layer.open({
		title:url,
	  	type: 2, 
...
function viewtext(url){
	var url = "./admin/viewtext.php?file=" + url;
	layer.open({
		title:'文本查看器',
	  	type: 2, 
	  	area: ['80%', '80%'],
...
function filehash(name,path){
	...
	//alert(file);
	$.post("./admin/hash.php",{file:file},function(data,status){
...
function delfile(id,filename,filepath){
	...
	layer.prompt({
		...
		$.post('./admin/delfile.php',{filepath:filepath,password:value},function(data,status){
